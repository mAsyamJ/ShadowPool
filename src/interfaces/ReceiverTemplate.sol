...
abstract contract ReceiverTemplate is IReceiver, Ownable {
  ...
  // Custom errors
  error InvalidForwarderAddress(); // Chainlink Forwarder
  error InvalidSender(address sender, address expected); // Chainlink Forwarder
  error InvalidAuthor(address received, address expected); // Workflow owner
  error InvalidWorkflowName(bytes10 received, bytes10 expected); // Workflow name
  error InvalidWorkflowId(bytes32 received, bytes32 expected); // Workflow ID
  error WorkflowNameRequiresAuthorValidation(); // Workflow name requires author validation
